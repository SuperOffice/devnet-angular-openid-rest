...
export class ContactDetailComponent implements OnInit {
  ...
  ngOnInit() {
    this.listService.getBusinessList().subscribe(businesses => {
      this.businessList = businesses;
    });

    this.listService.getCategoryList().subscribe(categories => {
      this.categoryList = categories;
    });

    this.listService.getCountryList().subscribe(countries => {
    ...
    const contactId = this.route.paramMap.subscribe((params: ParamMap) => {
      ...
      if (this.selectedContactId > 0) {
        ...
        this.contactSvc.createContact().subscribe(contact => {
          this.selectedContact = contact;
        });
      }
    });
