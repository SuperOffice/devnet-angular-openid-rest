...
export class SaleDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    this.listSvc.getSaleTypeList().subscribe(saleTypes => {
      this.saleTypeList = saleTypes;
    });
    this.getUserDefinedFieldTypes();

    const saleId = this.route.paramMap.subscribe((params: ParamMap) => {
      const id = params.get("id");

      this.selectedSaleId = id;
      if (this.selectedSaleId > 0) {
        this.saleSvc.getSale(this.selectedSaleId).subscribe(sale => {
          this.selectedSale = sale;
          this.setContactAndPersonFields();
        });
      } else {
        this.saleSvc.getSale(0).subscribe(sale => {
          this.selectedSale = sale;
          this.setContactAndPersonFields();
        });
      }
  ...
  saveSale() {
    this.saleSvc.saveSale(this.selectedSale).subscribe(
      sale => {
        this.selectedSale = sale;
      },
      error => {
  ...
  deleteSale(saleId) {
    this.saleSvc.deleteSale(saleId).subscribe(
      sale => {
        const ok = sale;
      },
      error => {
  ...
  getSaveTitle(): string {
    return this.selectedSale.SaleId > 0
      ? `PUT api/v1/Sale/${this.selectedSale.SaleId} (sale json in body)`
      : `POST api/v1/Sale (sale json in body)`;
  }
}
